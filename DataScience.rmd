...
g_competitors <- graph_from_data_frame(edge_weights, directed = FALSE)

# Setze die Gewichtungen der Kanten im Graphen
E(g_competitors)$weight <- edge_weights$weight

# Setze die Farben der Kanten basierend auf der Gewichtung
E(g_competitors)$color <- "red"

# Visualisiere das Netzwerk mit kleineren Knoten
plot(g_competitors, vertex.label = NA,
     vertex.size = 3,  # Kleinere Knoten
     edge.width = 2 * E(g_competitors)$weight,  # Gewichtung der Kanten
     edge.arrow.size = 1,
...
# Gehaltvergleich in Netzwerkzentren und Peripherien: 
# Überprüfen, ob zentralere Unternehmen tendenziell höhere oder niedrigere Gehälter bieten


## Gehaltsverteilung in Netzwerkzentren und Peripherien
### Cluster-Analyse
```{r}
# Detect communities
